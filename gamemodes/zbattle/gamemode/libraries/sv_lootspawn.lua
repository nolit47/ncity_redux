...
function hg.GenerateLoot(ply,ent,func)
	...
	local mul = traitor_opened and 1.5 or (low_karma_player and 0.75 or 1)
...
local functions_break = {
    ...
    ["Ammo"] = function(ent, ammo)
		local ammo = isnumber(ammo) and ammo or game.GetAmmoID(hg.ammotypes[ammo].name)
		local ammo = hg.ammotypes[game.GetAmmoName(ammo)].name
...
hook.Add("PropBreak", "LootSpawn", function(ply,ent)
	...
	for i = 0,amount-1 do
		...
		if not entName or entName == "" then continue end

		local huy = ents.Create(entName)
		if not IsValid(huy) then continue end
		huy.IsSpawned = true
		huy:SetPos(ent:GetPos())
		huy:Spawn()
		huy.init = true
...
hook.Add("PostCleanupMap", "addboxs", function()
	...
	timer.Simple(.5,function()
		...
		timer.Create("SpawnTheBoxes", 5, 0, function() hook_Run("Boxes Think") end)
	end)
end)
...
hook.Add("Boxes Think", "SpawnBoxes", function()
	...
	/*if not CurrentRound().noBoxes then
		for k, ply in ipairs(ents.FindInSphere(spawnPos,1000)) do
	...
	if (math.random(15) == 1) and not CurrentRound().noBoxes then
		...
		huy:SetPos(spawnPos)
		local randprop
		for model,tbl in RandomPairs(hg.loot_boxes) do
			if not tbl[3] then randprop = model break end
		end
		if not util.IsValidProp(randprop) then huy:Remove() return end
		huy:SetModel(randprop)
		huy:Spawn()
