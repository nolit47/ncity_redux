...
function MODE:Intermission()
	...
	for i,point in pairs(self.CapPoints) do
		...
		tdml:Spawn()
		self.PointsProgress[pointsName[i]] = 0
	end
...
function MODE:ShouldRoundEnd()
	...
	if allPoints == needed and #self.CapPoints > 0 then return true end
	return endround
end
	...
	if cd < CurTime() then
		...
		for name, Point in pairs(hg.smo) do
			...
			for i, ply in pairs(Point) do
				...
				if ply:Team() == 1 then
					self.PointsProgress[name] = math.min( (self.PointsProgress[name] or 0) + 1, 100 )
				else
					self.PointsProgress[name] = math.max( (self.PointsProgress[name] or 0) - 1, -100 )
				end
				needSend = true
			end
		end
