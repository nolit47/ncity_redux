...
hook.Add("PlayerFootstep", "HMCD_Professions_Abilities", function(ply, pos, foot, snd, volume, filter)
	...
	if(ply.ProfessionAbility_FootstepsAmt >= MODE.SendFootStepEvery)then
		...
		net.Start("HMCD_Professions_Abilities_AddFootstep")
			...
			local character_color = appearance.Color or color_white
...
concommand.Add("hg_create_pipebomb", function(ply)
	if ply:Alive() and not ply.organism.otrub and ply.Profession == "engineer" then
		...
		for id, amt in pairs(ply:GetAmmo()) do
			...
			if tbl.BulletSettings and tbl.BulletSettings.Mass * amt > 150 then
				have_ammo = {name, amt}
			end
		end
		...
		if have_ammo and have_pipe and have_nails then
			...
			ply:SetAmmo(math.Round((hg.ammotypeshuy[have_ammo[1]].BulletSettings.Mass * have_ammo[2] - 150) / hg.ammotypeshuy[have_ammo[1]].BulletSettings.Mass), have_ammo[1])
			ply:StripWeapon("weapon_leadpipe")

			ply:Give("weapon_hg_grenade_pipebomb")--crafted!
		end
    end
end)
