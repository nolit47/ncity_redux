...
function PANEL:Paint(w, h)
    ...
    surface.SetDrawColor(255, 255, 255, self.a * 3)

    for i = 1, (ybars2 + 1) do
        surface.DrawRect((sw / ybars2) * i - (CurTime() * 30 % (sw / ybars2)), 0, ScreenScale(1), sh)
    end

    for i = 1, (xbars2 + 1) do
        surface.DrawRect(0, (sh / xbars2) * (i - 1) + (CurTime() * 30 % (sh / xbars2)), sw, ScreenScale(1))
    end
...
vgui.Register("ZB_ScrappersGrid", PANEL, "EditablePanel")