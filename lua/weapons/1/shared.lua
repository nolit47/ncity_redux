...
function SWEP:CoreStep()
	...
	if SERVER and IsValid(owner) and ((self.cooldown_suiciding or 0) < CurTime()) then
		self.cooldown_suiciding = CurTime() + 0.05
		owner:SetNWBool("suiciding", owner.suiciding)
	end

	if CLIENT and IsValid(owner) then
		owner.suiciding = owner:GetNWBool("suiciding", false)
	end
...
function SWEP:GetAdditionalValues(closeanim)
	...
	local func = hg.postureFunctions2[self.reload and 0 or (self:IsSprinting() or huya) and (self:GetButtstockAttack() - CurTime() < -1) and ((ply.posture == 3 and 3) or (ply.posture == 3 and 3) or (self:IsPistolHoldType() and 3 or 3)) or ply.posture] or funcNil
	...
	self.pitch = Lerp(hg.lerpFrameTime(0.001,dtime), self.pitch, huypitch and 1 or 0)
...
function SWEP:InUse()
	...
	return ( (not ply.InVehicle || !ply:InVehicle()) && self:KeyDown(IN_USE)) || (ply.InVehicle && ply:InVehicle() && not self:KeyDown(IN_USE)) || (self.reload and self.reload > 0) || (self.reloadCoolDown and self.reloadCoolDown > CurTime()) || (IsValid(ply.OldRagdoll))
end
