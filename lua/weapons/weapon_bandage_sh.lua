...
if SERVER then
	function SWEP:Bandage(ent, bone)
		...
		if not bone then
			...
			for i = 1, #org.wounds do
				if self.modeValues[1] > 0 and #org.wounds > 0 then
					...
					local bone_name = ent:GetBoneName(org.wounds[1][4])
			...
			for i,tbl in pairs(org.wounds) do
				if ent:GetBoneName(tbl[4]) == bone then
			...
			for i = 1, #bonewounds do
				if self.modeValues[1] ~= 0 and #bonewounds > 0 then
					if org.wounds[bonewounds[1]] then
						...
						ent.bandaged_limbs = ent.bandaged_limbs or {}
						local bone_name = ent:GetBoneName(org.wounds[bonewounds[1]][4])
						if not ent.bandaged_limbs[bone_name] then
							ent.bandaged_limbs[bone_name] = true
							done = true
						end
		...
		if org.lleg == 1 and self.modeValues[1] >= amt then
		...
		if org.rleg == 1 and self.modeValues[1] >= amt then
		...
		if org.rarm == 1 and self.modeValues[1] >= amt then
		...
		if org.larm == 1 and self.modeValues[1] >= amt then
