-- "addons\\mcity\\lua\\weapons\\weapon_hands_sh.lua"
-- Retrieved by https://github.com/lewisclark/glua-steal
if SERVER then AddCSLuaFile() end
-- override attacks for furry_fists: no charge attack, use custom anims
function SWEP:DoPrimaryAttack()
	local owner = self:GetOwner()
	if not IsValid(owner) then return end
	
	-- if furry_fists, use custom anim set and skip charge attack
	if owner.PlayerClassName == "furry" then
		local anims = self:GetFurryAnimSet()
		self:PlayAnim(anims.attack, self.AnimTime1, false, nil, false)
		self:SetAttackType(1)
		self:SetLastAttack(CurTime() + self.AttackTime)
		self:SetAttackTime(self:GetLastAttack() + self.AttackTimeLength)
		self:SetAttackLength(self.AttackLen1)
		self:SetAttackWait(self.WaitTime1)
	...
	-- otherwise do normal primary attack (including charge)
	self.BaseClass.DoPrimaryAttack(self)
end
...
function SWEP:DoSecondaryAttack()
	...
	if owner.PlayerClassName == "furry" then
		local anims = self:GetFurryAnimSet()
		self:PlayAnim(anims.attack2, self.AnimTime2, false, nil, false)
		self:SetAttackType(2)
		self:SetLastAttack(CurTime() + self.AttackTime)
		self:SetAttackTime(self:GetLastAttack() + self.Attack2TimeLength)
		self:SetAttackLength(self.AttackLen2)
		self:SetAttackWait(self.WaitTime2)
		self:SetInAttack(true)
		if CLIENT and not self:IsLocal() and owner.AnimRestartGesture then
			owner:AnimRestartGesture(GESTURE_SLOT_ATTACK_AND_RELOAD, ACT_HL2MP_GESTURE_RANGE_ATTACK_SLAM, true)
		end
	...
	-- otherwise do normal secondary attack (including charge)
	self.BaseClass.DoSecondaryAttack(self)
end
...
function SWEP:Think()
	...
	-- reset assimilation progress when not carrying a player
	if owner.PlayerClassName == "furry" and (not self.CarryEnt or not self.CarryEnt:IsPlayer()) then
		owner.assimProgress = 0
	end
	
	-- idle animation override for furry_fists
	if owner.PlayerClassName == "furry" and self:GetFists() and not self:GetInAttack() and not self:GetBlocking() then
		local anims = self:GetFurryAnimSet()
		-- only play idle if not already playing it
		if not self.lastIdleAnim or self.lastIdleAnim ~= anims.idle then
			self:DoBFSAnimation(anims.idle, 1)
			self.lastIdleAnim = anims.idle
		end
	else
		self.lastIdleAnim = nil
	end
end
if SERVER then util.AddNetworkString("FurryAssimProgress") end
...
SWEP.BlockStaminaCost = 1             -- lower stamina cost per blocked damage (was 12)
...
SWEP.RagdollCarryForceMultiplier = 1.5 -- Make ragdolls much easier to carry
...
-- Attack timing configuration (faster charge attack)
SWEP.AttackTime = 0.15 -- Faster than default 0.2
SWEP.AttackTimeLength = 0.12 -- Faster than default 0.15
SWEP.AnimTime1 = 0.6 -- Slightly faster animation
SWEP.WaitTime1 = 0.4 -- Faster wait time
SWEP.AttackLen1 = 45 -- Slightly shorter attack length

SWEP.AnimTime2 = 0.5 -- Faster secondary animation  
SWEP.WaitTime2 = 0.3 -- Faster secondary wait time
SWEP.AttackLen2 = 40 -- Slightly shorter secondary attack length
...
if CLIENT then
	...
	function SWEP:DrawWorldModel()
		...
		if owner.PlayerClassName == "furry" then
			...
			WorldModel:SetModelScale(1.2,0)
		else
			WorldModel:SetModel(self.WorldModel)
			WorldModel:SetModelScale(1,0)
		end
...
-- furry claw animation set helper
function SWEP:GetFurryAnimSet()
	return {
		idle = "fists_idle_01",
		deploy = "fists_draw",
		attack = "fists_right",
...
function SWEP:Deploy()
	if not IsFirstTimePredicted() then
		...
		if owner.PlayerClassName == "furry" then
			local anims = self:GetFurryAnimSet()
			self:DoBFSAnimation(anims.deploy,1)
		else
	...
	if SERVER then
		...
		if (IsValid(tr.Entity)) and self:CanPickup(tr.Entity) and not tr.Entity:IsPlayer() then
			...
			--if Dist < self.ReachDistance then
				sound.Play("physics/body/body_medium_impact_soft" .. math.random(1, 7) .. ".wav", self:GetOwner():GetShootPos(), 65, math.random(90, 110))
	...
	if IsValid(phys) then
		...
		if SERVER then
			...
			if ply:KeyDown(IN_ATTACK) and not ply.organism.superfighter then
				-- furry assimilation while carrying
				if ply.PlayerClassName == "furry" and self.CarryEnt:IsPlayer() then
					local victim = self.CarryEnt
					ply.assimProgress = (ply.assimProgress or 0) + FrameTime()
					net.Start("FurryAssimProgress")
						net.WriteFloat(ply.assimProgress)
					net.Send(ply)
					if ply.assimProgress >= 5 then
						victim.PlayerClassName = "furry"
						victim:SetModel("models/eradium/protogen_player.mdl")
						victim:EmitSound("furry/assimilation"..math.random(1,3)..".wav", 75)
						victim:SetNetVar("assimilated", true)
						victim:ViewPunch(Angle(math.random(-10,10),math.random(-10,10),0))
						ply.assimProgress = 0
					end
					return -- skip throw while assimilating
				end
			...
			if ply:KeyDown(IN_ATTACK) and ply.organism.superfighter then
				...
				if ply.organism.rage and ply.organism.rage > 0 then
					rageMul = 1 + ply.organism.rage * 6 -- 0 rage = 1x, 1 rage = 7x
				end
				superThrowForce = superThrowForce * rageMul
...
function SWEP:Think()
	...
	if self:GetFists() then
		...
		-- Commented out to allow fist raising while running
	...
	-- Commented out to allow proper hold type while running
...
function SWEP:PrimaryAttack(forcespecial)
	...
	-- Commented out to allow punching while running
	...
	if CLIENT and self.IsLocal and self:IsLocal() then
		if special_attack then
			...
			ViewPunch(Angle(0, -2, 0)) -- Initial right position
			timer.Simple(0.5, function() -- Timing matches Attack_Charge_End
				ViewPunch(Angle(0, 10, 0)) -- Right hook sideways motion (increased range and wideness)
			end)
		else
	...
	if special_attack then
		...
		local owner = self:GetOwner()
		if owner.PlayerClassName == "furry" then
			-- Use furry-specific charge animations
			local anims = self:GetFurryAnimSet()
			self:DoBFSAnimation(anims.attack, 0.85) -- Use right punch for charge begin
			timer.Simple(0.5, function()
				if IsValid(self) then
					self:DoBFSAnimation(anims.attack, 1) -- Use right punch for charge end
					-- Trigger fist raising effect during charge end
					self.fistRaising = true
					-- Attack happens during charge end for special attacks
					if SERVER then
					...
					-- Lerp fists back down after charge end completes
		...
		local owner = self:GetOwner()
		if owner.PlayerClassName == "furry" then
			-- Use furry-specific animations
			local anims = self:GetFurryAnimSet()
			if rand then
				self:DoBFSAnimation(anims.attack, 0.8) -- right punch
			else
				self:DoBFSAnimation(anims.attack2, 0.8) -- left punch
			end
		else
			-- Regular punches use the default animations
			if rand then
				self:DoBFSAnimation("Attack_Quick",0.8) -- right punch
			else
				self:DoBFSAnimation("Attack_Quick2",0.8) -- left punch
			end
		end
...
function SWEP:AttackFront(special_attack,rand)
	...
	-- Increased right hook range for better reach
	local Ent, HitPos, _, physbone = WhomILookinAt(owner, .3, special_attack and 60 or 45)
	...
	if IsValid(Ent) or (Ent and Ent.IsWorld and Ent:IsWorld()) then
		...
		if self:IsEntSoft(Ent) then
			...
			if Ent:IsPlayer() and IsValid(Ent:GetActiveWeapon()) and Ent:GetActiveWeapon().GetBlocking and Ent:GetActiveWeapon():GetBlocking() and not RagdollOwner(Ent) then
				sound.Play("physics/body/body_medium_impact_hard" .. math.random(1, 6) .. ".wav", HitPos, 65, math.random(90, 110))
			else
				sound.Play("physics/body/body_medium_impact_hard" .. math.random(1, 6) .. ".wav", HitPos, 65, math.random(90, 110))
			end
		else
			sound.Play("physics/body/body_medium_impact_hard" .. math.random(1, 6) .. ".wav", HitPos, 65, math.random(90, 110))
			-- 50/50 chance of bleeding when punching hard surfaces (walls)
			if not self:IsEntSoft(Ent) then
				if math.random(1, 2) == 1 then -- 50% chance
					-- Cause bleeding instead of just pain
					local bleedAmount = special_attack and math.Rand(12, 20) or math.Rand(5, 12)
					hg.organism.AddWoundManual(owner, bleedAmount, vector_origin, AngleRand(), owner:LookupBone("ValveBiped.Bip01_"..(rand and "R" or "L").."_Hand"), CurTime())
				else
					-- Regular pain damage
		...
		-- Speed-based damage multiplier: faster movement = more damage
		local ownerVelocity = owner:GetVelocity():Length()
		local speedMultiplier = 1 + math.Clamp(ownerVelocity / 300, 0, 1.5) -- Max 2.5x damage at high speeds
		
		-- Increased damage: heavier punches pack more punch
		local DamageAmt = (math.random(6, 9) * (special_attack and 4 or 1)) * (self.DamageMul or 1) * speedMultiplier
		local ent = Ent
		local vec = AimVec
		...
		-- Rage mode damage multiplier (1x to 3x based on rage level)
		local rageMul = 1
		if owner.organism.rage and owner.organism.rage > 0 then
			rageMul = 1 + owner.organism.rage * 2 -- 0 rage = 1x, 1 rage = 3x
			Mul = Mul * rageMul
		end
		...
		if owner.PlayerClassName == "furry" then
			...
			Mul = Mul * 1.5
			-- furry slash sounds
			owner:EmitSound("furry/claw_miss"..math.random(1,4)..".wav", 65, math.random(95,105))
			if Ent:IsPlayer() and Ent.PlayerClassName == "furry" and (owner.nextLick or 0) < CurTime() then
				-- lick-heal fellow furry
				owner:EmitSound("furry/lick"..math.random(1,3)..".wav", 60)
				if owner.organism then
					owner.organism.pain = math.max(owner.organism.pain - 8, 0)
				end
				owner.nextLick = CurTime() + 4
			end
			-- override flesh-hit sound to use knife stab table
			if Ent:IsPlayer() or Ent:IsRagdoll() then
				...
				sound.Play(stabSounds[math.random(#stabSounds)], HitPos, 75, math.random(90,110), 1)
			end
		end
		...
		-- furry claw decals
		if owner.PlayerClassName == "furry" and Ent:IsPlayer() then
			local decal = "FadingScorch"
			util.Decal(decal, HitPos + tr.HitNormal * 5, HitPos - tr.HitNormal * 5)
		end
...
function SWEP:ShoveAttack()
	...
	if owner.PlayerClassName == "furry" then
		-- Use furry-specific shove animation (right punch)
		local anims = self:GetFurryAnimSet()
		self:DoBFSAnimation(anims.attack, 0.9)
	else
		-- Use default shove animation
	...
	-- Calculate speed-based intensity multiplier (significantly reduced for balanced gameplay)
	local ownerVelocity = owner:GetVelocity():Length()
	local speedMultiplier = 1 + math.Clamp(ownerVelocity / 400, 0, 0.3) -- Up to 1.3x intensity when running fast
	local isRunning = ownerVelocity > 200 -- Consider running if moving faster than 200 units/sec
	...
	if IsValid(Ent) or (Ent and Ent.IsWorld and Ent:IsWorld()) then
		...
		-- Speed-based force calculation (significantly reduced base force for balanced gameplay)
		local SelfForce = 140 * speedMultiplier -- Base force multiplied by speed
		local isPlayerTarget = Ent:IsPlayer()
		local isRagdollTarget = Ent:IsRagdoll() or hg.RagdollOwner(Ent)
		
		-- Check if pushing from behind for extra intensity
		local isBehindPush = false
		if isPlayerTarget then
			local targetForward = Ent:GetAngles():Forward()
			local pushDirection = (Ent:GetPos() - owner:GetPos()):GetNormalized()
			local dot = targetForward:Dot(pushDirection)
			isBehindPush = dot > 0.3 -- Behind if dot product > 0.3
		end
		
		if self:IsEntSoft(Ent) then
			sound.Play("physics/body/body_medium_impact_hard" .. math.random(1, 6) .. ".wav", HitPos, 70, math.random(85, 105))
		else
		...
		-- Enhanced damage based on speed and behind push
		local baseDamage = math.random(1, 3)
		if isBehindPush then
			baseDamage = baseDamage * 2 -- Double damage from behind
		end
		local DamageAmt = baseDamage * math.min(speedMultiplier, 2) -- Cap damage multiplier at 2x
		...
		-- Enhanced ragdoll chance calculation
		local ragdollChance = 0.1 -- Base 10% chance
		local shouldRagdoll = false
		local lowStaminaRagdoll = false
		
		if isPlayerTarget and Ent.organism then
			local stamina = Ent.organism.stamina[1] or 100
			local blood = Ent.organism.blood or 100
			
			-- Low stamina ragdoll (collapse in place, no knockback)
			if stamina < 30 then
				lowStaminaRagdoll = true
				shouldRagdoll = true
			else
				-- Speed-based ragdoll chance when running (significantly reduced)
				if isRunning then
					ragdollChance = ragdollChance + 0.15 -- +15% chance when running
				end
				
				-- Behind push gets very high ragdoll chance
				if isBehindPush then
					ragdollChance = ragdollChance + 0.7 -- +70% chance from behind (total 80-90%)
				end
				
				-- Original weak condition
				if stamina < 95 or blood < 3500 then
					ragdollChance = ragdollChance + 0.3 -- +30% for weak targets
				end
				
				-- Roll for ragdoll
				shouldRagdoll = math.random() < ragdollChance
			end
		end
		...
		if IsValid(Phys) then
			-- Apply push force (modified for low stamina targets)
			if Ent:IsPlayer() then 
				local pushVel
				if lowStaminaRagdoll then
					-- low stamina: tiny shove, then ragdoll
					pushVel = AimVec * 50
					Ent:SetVelocity(pushVel)
					-- preserve momentum on ragdoll
					timer.Simple(0.1, function()
						if IsValid(Ent) and Ent:IsPlayer() and Ent:Alive() then
							local ragdoll = hg.Fake(Ent)
							if IsValid(ragdoll) then
								for i = 0, ragdoll:GetPhysicsObjectCount() - 1 do
									local phys = ragdoll:GetPhysicsObjectNum(i)
									if IsValid(phys) then phys:SetVelocity(pushVel) end
								end
							end
					...
					-- normal push: ensure at least ragdoll threshold speed
					local pushForce = SelfForce * 2.5
					if isBehindPush then
						pushForce = pushForce * 1.5 -- bonus from behind
					end
					if pushForce < 500 then pushForce = 500 end -- match ragdoll trigger speed
					pushVel = AimVec * pushForce
					Ent:SetVelocity(pushVel)
					Ent.PreRagdollVelocity = pushVel -- for any delayed ragdoll
				end
				
				-- view punch scales with shove
				local viewPunchIntensity = 15
				if isBehindPush then viewPunchIntensity = viewPunchIntensity * 1.8 end
				if isRunning then viewPunchIntensity = viewPunchIntensity * speedMultiplier end
				Ent:ViewPunch(Angle(-viewPunchIntensity, math.random(-viewPunchIntensity/2, viewPunchIntensity/2), math.random(-5, 5)))
				
				-- Enhanced collision detection for ragdolling (skip for low stamina targets as they're already ragdolled)
				if not lowStaminaRagdoll and not IsValid(Ent.FakeRagdoll) and not RagdollOwner(Ent) and Ent:IsPlayer() and Ent:Alive() then
					-- Enhanced collision detection with higher chance for intense pushes
					local timerName = "ShoveCollision_" .. Ent:SteamID64()
					local checkCount = 0
					local maxChecks = math.ceil(10 * speedMultiplier) -- Longer check time for faster pushes
					...
					timer.Create(timerName, 0.1, maxChecks, function()
						...
						-- Enhanced ragdoll chance when hitting props
						if hitProp then
							-- Higher chance to ragdoll based on push intensity (reduced running bonus)
							local propRagdollChance = 0.6 -- Base 60% chance
							if isRunning then
								propRagdollChance = propRagdollChance + 0.15 -- +15% when running (reduced from 30%)
							end
							if isBehindPush then
								propRagdollChance = propRagdollChance + 0.3 -- +30% from behind
							end
							
						if math.random() < propRagdollChance or shouldRagdoll then
							-- delayed ragdoll on prop hit; carry shove momentum
							timer.Simple(0.1, function()
								if IsValid(Ent) and Ent:IsPlayer() and Ent:Alive() then
									local ragdoll = hg.Fake(Ent)
									local vel = Ent.PreRagdollVelocity or (AimVec * 500)
									if IsValid(ragdoll) then
										for i = 0, ragdoll:GetPhysicsObjectCount() - 1 do
											local phys = ragdoll:GetPhysicsObjectNum(i)
				...
				-- Enhanced ragdoll system for weak/intense pushes (skip low stamina as already handled)
				if shouldRagdoll and not lowStaminaRagdoll and not RagdollOwner(Ent) then
					-- no guaranteed ragdoll from behind; only by intensity
					if ragdollChance > 0.8 then
						timer.Simple(0.1, function()
							if IsValid(Ent) and Ent:IsPlayer() and Ent:Alive() then
								local ragdoll = hg.Fake(Ent)
								local vel = Ent.PreRagdollVelocity or (AimVec * 500)
						...
						hg.LightStunPlayer(Ent, 2) -- 2 second stun for less intense pushes
					end
				end
			end
...
function SWEP:DoBFSAnimation(anim,time)
	if CLIENT and IsValid(self:GetWM()) then
		-- Debug: Check if animation exists in model
		local wm = self:GetWM()
...
-- PlayAnim method for compatibility with furry animations
function SWEP:PlayAnim(anim, time, cycling, callback, reverse, sendtoclient)
	-- Use DoBFSAnimation for actual animation playback
	self:DoBFSAnimation(anim, time or 1)
	
	-- Handle callback if provided
	if callback then
		timer.Simple(time or 1, function()
			if IsValid(self) then
				callback(self)
			end
		end)
...
if CLIENT then
	net.Receive("play_anim",function()
		...
		local owner = self:GetOwner()
		if anim == "Attack_Quick2" or anim == "Attack_Quick" or anim == "Attack_Charge_Begin" or anim == "Attack_Charge_End" or anim == "Shove" or (owner.PlayerClassName == "furry" and (anim:find("attack"))) then
			local isRight = (anim == "Attack_Quick" or anim == "Attack_Charge_Begin" or anim == "Attack_Charge_End" or anim == "Shove" or (owner.PlayerClassName == "furry" and anim == "anims.attack"))
			owner:AddVCDSequenceToGestureSlot(GESTURE_SLOT_ATTACK_AND_RELOAD,owner:LookupSequence(isRight and "range_fists_r" or "range_fists_l"),0,true)
		end
		end
	end)
else
...
if SERVER then
    -- removed progressive block stamina hook: it double-charged stamina on hit
