-- "addons\\homigrad\\lua\\homigrad\\organism\\tier_1\\cl_statistics.lua"
...
local inf
local attacker
local function ensureClientModels()
	if not IsValid(csmodel) then
		csmodel = ClientsideModel("models/Humans/group01/Female_03.mdl", RENDERMODE_TRANSCOLOR)
		csmodel:SetNoDraw(true)
	end

	if not IsValid(skiletmodel) then
		skiletmodel = ClientsideModel("models/player/sniper elite 3/skeleton_playermodel.mdl", RENDERMODE_TRANSCOLOR)
		skiletmodel:SetNoDraw(true)
		skiletmodel:SetBodyGroups("1110")
		skiletmodel:AddEffects(EF_BONEMERGE)
	...
	if not IsValid(bulletmodel) then
		bulletmodel = ClientsideModel("models/bullets/w_pbullet1.mdl", RENDERMODE_TRANSCOLOR)
		bulletmodel:SetNoDraw(true)
	end
end
...
local function startPlayingHit(i)
	...
	if not bone0 then return end
	if not IsValid(csmodel) then return end

	-- bone0 is expected to be a matrix; guard its use
	if bone0.GetTranslation then
		csmodel:SetPos(bone0:GetTranslation())
	else
		-- fallback to ent position if matrix is invalid
		if IsValid(ent) then
