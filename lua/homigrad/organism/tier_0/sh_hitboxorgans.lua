-- "addons\\homigrad\\lua\\homigrad\\organism\\tier_0\\sh_hitboxorgans.lua"
...
function hg.organism.ShootMatrix(ent, organs)
	local boxs = {}
	if type(organs) ~= "table" then organs = {} end
	...
	for nameBone, orgs in pairs(organs) do
		...
		pos = matrix:GetTranslation()
		ang = matrix:GetAngles()
		for key, organ in pairs(orgs) do
			--print(key,organ[1])
