...
function DrawAccesories(ply,ent,accessories,accessData,islply,force)
	...
	--print(CurTime(), ent)
	-- allow runtime skin override from preview/menu
	if ent.AccessorySkins and isnumber(ent.AccessorySkins[accessories]) then
		model:SetSkin( ent.AccessorySkins[accessories] )
	end

	-- allow runtime bodygroups override from preview/menu
	if ent.AccessoryBodygroups and ent.AccessoryBodygroups[accessories] then
		for id, val in pairs(ent.AccessoryBodygroups[accessories]) do
			model:SetBodygroup(id, val)
		end
	end
...
function DrawAppearance(ent, ply)
    ...
    -- sync runtime chosen skins/bodygroups for accessories from server appearance
    ent.AccessorySkins = ent:GetNetVar("AccessorySkins", ent.AccessorySkins)
    ent.AccessoryBodygroups = ent:GetNetVar("AccessoryBodygroups", ent.AccessoryBodygroups)
