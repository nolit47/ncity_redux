...
function CLASS.Guilt(self, victim)
    ...
    if victim.PlayerClassName == self.PlayerClassName then
        return 1
    end
end
...
if SERVER then
	util.AddNetworkString("FurCrack")
...
if CLIENT then
	...
	hook.Add("Org Think", "regenerationfurry", function(owner, org, timeValue)
		...
		for i, wound in pairs(org.wounds) do
			wound[1] = math.max(wound[1] - timeValue * 0.2,0)
		end
		...
		org.internalBleed = math.max(org.internalBleed - timeValue * 0.2, 0)
		
		local regen = timeValue / 120
