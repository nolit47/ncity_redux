...
function hg._DeprecatedDoTPIK(ply, ent, rhmat, lhmat)
    ...
    local angasd = eyeang//ent:GetBoneMatrix(ent:LookupBone("ValveBiped.Bip01_Head1")):GetAngles()
        ...
        if ply:InVehicle() then
            ...
            if(IsValid(Car))then
                ...
                if bone and Car:GetBoneMatrix(bone) and wpn and not wpn.reload then
                    ...
                    hg.DragLeftHand_Ex(ent,wpn,pos,ang,adjust[2])
                    if not IsValid(wpn) and adjust[3] then
                        ...
                        ply.lerp_rh = 1
                        wpn.rhandik = true
                        hg.DragRightHand_Ex(ent,wpn,pos,ang,adjust[4])
                    end
                end
            end
...
function hg.DragRightHand_Ex(ply,self,pos,ang,anglh)
    if not IsValid(ply) then return end   	
