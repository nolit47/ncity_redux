...
local function funca(ply, txt)
	...
	local bHasMassiveBrainDamage = ply.organism.brain > 0.1
	...
	if bUnintelligeble then
		...
		for i = 2, len do
			if math.random(bHasMassiveBrainDamage and 2 or 3) == 1 then
				local old = utf8.sub(txt, i - 1, i - 1)
				txt = hg.replace_by_index(txt, i - 1, utf8.sub(txt, i, i) )
				txt = hg.replace_by_index(txt, i, old )
			end
		end

		if bHasMassiveBrainDamage then
			if math.random(2) == 1 then txt = hg.utf8_reverse(utf8.codes(txt), utf8.len(txt)) end

			for i = 1, len do
				if math.random(3) == 1 then
					txt = hg.replace_by_index(txt, i, utf8.force((math.random(1,2) > 1 and "m" or "b")) )
				end
			end
		end
	end
