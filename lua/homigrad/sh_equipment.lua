...
if CLIENT then
	...
	hook.Add( "Think", "NVGEnabling", function( )
		if input.IsKeyDown(KEY_C) and input.IsKeyDown(KEY_E) and not gui.IsGameUIVisible() and not IsValid(vgui.GetKeyboardFocus()) then
	...
	hook.Add("OnNetVarSet", "asdasdsad_equipment", function(index, key, var)
		if key == "Armor" then
			if IsValid(hg.armorMenuPanel) and hg.armorMenuPanel.RefreshTbl and Entity(index) == lply then
	...
	CreateMenu = function()
		...
		lbl.Paint = function(self, w, h)
			draw.SimpleText("LMB - Remove armor", "ZCity_Tiny", w * 0.5, h * 0.5, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		end
		...
		function frame:RefreshTbl()
			...
			for k, v in pairs(tblcpy) do
				if !hg.armorNames[v] then continue end
				local but = vgui.Create("DButton")
				but:SetText( hg.armorNames[v] )
				but:Dock( TOP )
